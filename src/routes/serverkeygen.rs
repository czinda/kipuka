...
pub async fn post_serverkeygen(
    ...
    if let Some(ref dogtag_pool) = state.dogtag {
        ...
        let mut rand = [0u8; 24];
        openssl::rand::rand_bytes(&mut rand).map_err(|e| KipukaError::Ca(format!("rand: {e}")))?;
        ...
        let (cert_der, private_key_der) = if let Some(ref p12_b64) = sskg_result.pkcs12_b64 {
            ...
            let cert = if let Some(cert) = &parsed.cert {
                cert.to_der()
                    .map_err(|e| KipukaError::Ca(format!("cert DER: {e}")))?
            } else {
                sskg_result
                    .certificate_der
                    .clone()
                    .ok_or_else(|| KipukaError::Ca("No certificate in SSKG response".into()))?
            };
            (cert, key_der)
        } else {
            ...
            let key_entry = keys.first().ok_or_else(|| {
                KipukaError::Ca("No keys found on KRA after SSKG enrollment".into())
            })?;
            ...
            let key_der = kra_client
                .recover_key_no_cert(&key_entry.key_id)
                .await
