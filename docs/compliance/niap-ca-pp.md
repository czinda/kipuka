...
| FAU_STG.1 | Protected Audit Trail Storage | Done | Audit events form a tamper-evident hash chain: each row stores `record_hash = H(prev_hash ‖ RS(0x1e) ‖ canonical_bytes)` (`src/audit/mod.rs`). `H` is SHA-256 by default, or HMAC-SHA256 keyed by `[audit].integrity_key` when `[audit].signed = true` (key resolved via the `SecretRef` backends). Any in-place edit, deletion, or reordering of a row breaks the chain and is detected by `verify_chain()`, which reports the offending row id (`broken_at`). Schema migration v4 adds the `prev_hash`/`record_hash` columns. Database access remains INSERT-only for the application role and the file-based log uses append-only mode. |
| FAU_STG.4 | Prevention of Audit Data Loss | Done | `audit::record()` returns a `Result`; a failed durable write sets a halt flag when `[audit].fail_closed = true` (default), and `AppState::ensure_audit_available()` guards every issuance endpoint so no certificate is minted without a durable audit record (HTTP 503 while halted). Configurable overflow behavior via `[audit].overflow_policy`: `halt` fails closed at `[audit].max_rows`; `drop_oldest` prunes the oldest rows under the chain lock. Security violations (security.violation events) continue to trigger operation rejection. |
...
| SFR | Title | Status | kipuka Implementation |
|-----|-------|--------|----------------------|
| FPT_FLS.1 | Fail Secure | Done | On loss of the audit trail -- a durable write failure, or storage exhaustion under `overflow_policy = "halt"` -- the server fails secure: the halt flag is set and all certificate-issuing endpoints reject requests via `ensure_audit_available()` (HTTP 503) until an operator intervenes, so no security-relevant action proceeds unaudited. Governed by `[audit].fail_closed` (default `true`). |
